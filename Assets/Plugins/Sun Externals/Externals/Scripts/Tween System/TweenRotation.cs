    ...
    public class TweenRotation : TweenBase
    {
        ...
        private void CheckCache(Vector3 resetValue)
        {
            switch (TargetType)
            {
                ...
                case TargetType.RectTransform:
                    _startValue = _rectTransform.localRotation;
                    break;
            }

            if (ResetWhenPlay) _startValue = Quaternion.Euler(resetValue);
        }
        ...
        private void SetValue(Quaternion value)
        {
            switch (TargetType)
            {
                case TargetType.Transform:
                    ...
                    break;
                case TargetType.RectTransform:
                    _rectTransform.localRotation = value;
                    break;
            }
        }
        ...
        public override void TweenReverse()
        {
            ...
            void SetTween()
            {
                ...
                {
                    MainTween = TweenHelper.AddTweener(_startValue, _startValue.eulerAngles + (ToValue - FromValue), Duration, SetValue)
                        .OnStart(OnStartReverse)
                        .OnComplete(OnCompleteReverse)
                        .SetEaseOrCurve(usingEase, ease, curve)
                        .SetDelay(DelayEvent.HaveOption(Event.Reverse) ? DelayTime : 0);
        ...
        public override void ResetToFromValue() => SetValue(Quaternion.Euler(FromValue));

        public override void ResetToToValue() => SetValue(Quaternion.Euler(ToValue));
